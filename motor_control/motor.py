...
# End property decorators
    ...
    def speed(self, signed_speed):
        if signed_speed >= 0:
            self._gpio_pi.write(self._motor_direction, 1)
            self._gpio_pi.set_PWM_dutycycle(self._motor_enable, signed_speed)
        else:
            self._gpio_pi.write(self._motor_direction, 0)
            self._gpio_pi.set_PWM_dutycycle(self._motor_enable, abs(signed_speed))
